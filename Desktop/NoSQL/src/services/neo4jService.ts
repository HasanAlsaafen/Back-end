  ...
  try {
    await session.run(
      `MATCH (s:Student {id: $studentId}), (c:Course {id: $courseId})
       MERGE (s)-[r:TAKES]->(c)
       SET r.semester = $semester, r.grade = $grade`,
      { studentId, courseId, ...props }
    );
    return { message: 'Takes relationship created/updated' };
  } finally {
    await session.close();
  }
};
...
// COMPLEX QUERY
// COMPLEX QUERY 2: Find Peers (Shared Courses >= 1)
export const findRecommendedPeers = async (studentId: string): Promise<{ peer: NodeProperties; sharedCount: number }[]> => {
    const session = getSession();
    try {
      const result = await session.run(
        `
        MATCH (s:Student {id: $studentId})-[:TAKES]->(c:Course)<-[:TAKES]-(peer:Student)
        WHERE s.id <> peer.id
        WITH peer, count(DISTINCT c) AS sharedCourses
        WHERE sharedCourses >= 1
        RETURN peer, sharedCourses
        ORDER BY sharedCourses DESC, peer.name ASC
        LIMIT 10
        `,
        { studentId }
      );
      ...
      return result.records.map((record: Record) => ({
        peer: record.get('peer').properties,
        sharedCount: typeof record.get('sharedCourses') === 'object' ? record.get('sharedCourses').toNumber() : record.get('sharedCourses')
      }));
    } finally {
      await session.close();
    }
  ...
  // COMPLEX QUERY 3: Suggested Mentors (Instructors of courses in the same department as student's takes)
  export const findSuggestedMentors = async (studentId: string): Promise<{ instructor: any; commonCourses: string[] }[]> => {
    const session = getSession();
    try {
      const result = await session.run(
        `
        MATCH (s:Student {id: $studentId})-[:TAKES]->(c:Course)<-[:TEACHES]-(i:Instructor)
        WITH i, collect(c.name) AS courses
        RETURN i { .name, .title } AS instructor, courses
        UNION
        MATCH (s:Student {id: $studentId})-[:TAKES]->(c:Course)-[:PREREQUISITE_OF]->(next:Course)<-[:TEACHES]-(i:Instructor)
        WITH i, collect(next.name) AS courses
        RETURN i { .name, .title } AS instructor, courses
        `,
        { studentId }
      );
      ...
      return result.records.map((record: Record) => ({
        ...
        commonCourses: record.get('courses')
      }));
    } finally {
      await session.close();
    }
  ...
  export const getDepartmentInfluence = async () => {
    const session = getSession();
    try {
      const result = await session.run(
        `MATCH (d:Department)
         OPTIONAL MATCH (d)<-[:BELONGS_TO]-(c:Course)
         OPTIONAL MATCH (c)<-[:TAKES]-(s:Student)
         OPTIONAL MATCH (d)<-[:MEMBER_OF]-(i:Instructor)
         RETURN d.name AS dept, 
                count(DISTINCT c) AS courseCount, 
                count(DISTINCT s) AS studentCount,
                count(DISTINCT i) AS instructorCount
         ORDER BY studentCount DESC`
      );
      return result.records.map((r: Record) => ({
        dept: r.get('dept'),
        courseCount: typeof r.get('courseCount') === 'object' ? r.get('courseCount').toNumber() : r.get('courseCount'),
        studentCount: typeof r.get('studentCount') === 'object' ? r.get('studentCount').toNumber() : r.get('studentCount'),
        instructorCount: typeof r.get('instructorCount') === 'object' ? r.get('instructorCount').toNumber() : r.get('instructorCount')
      }));
    } finally {
      await session.close();
    }
